...
EMAIL_ADDRESS="[removed for privacy]"
...
#PARITY2_FILE=`grep -v '^$\|^\s*\#' /etc/snapraid.conf | grep snapraid.2-parity | head -n 1 | cut -d " " -f2`
...
#if [ ! -e $PARITY2_FILE ]; then
#  echo "[`date`] ERROR - 2-Parity file ($PARITY2_FILE) not found!"
#  echo "ERROR - 2-Parity file ($PARITY2_FILE) not found!" >> $TMP_OUTPUT
#  exit 1;
#fi
